...
streamThread_pv_base::streamThread_pv_base(sc_module_name& module_name) : 
  streamThread_pv_base_mb_compatibility(module_name),
...
void streamThread_pv_base::print_parameters()
{
  const char* name;
...
streamThread_machine_arch::streamThread_machine_arch()
    : MachineArch(2, 4) {
    ...
    void streamThread_machine_arch::transaction_instance__slave_a__READ
      (
    ...
    void streamThread_machine_arch::transaction_instance__slave_a__WRITE
      (
        sc_dt::uint64 address,
        sc_dt::uint64 size,
        sc_dt::uint64 data, 
    ...
    void streamThread_machine_arch::transaction_instance__master_1__READ
      (
        sc_dt::uint64 address,
        sc_dt::uint64 size,
        sc_dt::uint64 data, 
    ...
    void streamThread_machine_arch::transaction_instance__master_1__WRITE
      (
        sc_dt::uint64 address,
        sc_dt::uint64 size,
        sc_dt::uint64 data, 
...
void streamThread_machine_arch::startTransaction(unsigned portIndex, unsigned transactionIndex, sc_dt::uint64 currentTime) {
  switch(portIndex) {
    case(0) : break;
    case(1) : break;
    default: break;
...
VariableBase* streamThread_machine_arch::getVariableBasePtrByName(const char* name) {
  if (!strcmp(name, "TP1")) {
    return &TP1;
  }
  if (!strcmp(name, "TP2")) {
...
Register* streamThread_machine_arch::getRegisterByName(const char* name) {
  return 0;
}
void streamThread_machine_arch::reset() { 
    MachineArch::reset();
}
bool streamThread_machine_arch::getAddressRange(unsigned portIndex, sc_dt::uint64& begin, sc_dt::uint64& end) {
    switch(portIndex) {
        case(0) :   return false;
        case(1) :   return false;
        default: break;
...
void streamThread_machine_arch::callTransaction(unsigned portIndex, unsigned transactionIndex, sc_dt::uint64 address, sc_dt::uint64 size, sc_dt::uint64 data, bool has_been_error, sc_dt::uint64 currentTime) {
    switch(portIndex) {
        case(0) : { //port: slave_a
...
void streamThread_t_base::schedule_generic_READ
(long port_index,
 sc_dt::uint64 delay,
...
void streamThread_t_base::schedule_generic_WRITE
(long port_index,
 sc_dt::uint64 delay,
 long& transactionSize,
...
void streamThread_t_base::start_transaction_generic_READ
...
void streamThread_t_base::end_transaction_generic_READ
(esl::include::TRequest* request,
 unsigned port_index,
 sc_dt::uint64 startTime,
 sc_dt::uint64 endTime,
...
void streamThread_t_base::start_transaction_generic_WRITE
(esl::include::TRequest* request,
 unsigned port_index,
 sc_dt::uint64 startTime,
 sc_dt::uint64 endTime,
...
void streamThread_t_base::end_transaction_generic_WRITE
(esl::include::TRequest* request,
 unsigned port_index,
 sc_dt::uint64 startTime,
 sc_dt::uint64 endTime,
...
streamThread_t_base::TransactionCallbackFunctionPointer streamThread_t_base::getTransactionCallback(unsigned callbackIndex) {
  static TransactionCallbackFunctionPointer funcArray[] = {
    static_cast<TransactionCallbackFunctionPointer>(&streamThread_t_base::start_transaction_generic_READ),
    static_cast<TransactionCallbackFunctionPointer>(&streamThread_t_base::start_transaction_generic_WRITE),
    static_cast<TransactionCallbackFunctionPointer>(&streamThread_t_base::start_transaction_generic_READ),
    static_cast<TransactionCallbackFunctionPointer>(&streamThread_t_base::start_transaction_generic_WRITE),
    static_cast<TransactionCallbackFunctionPointer>(&streamThread_t_base::end_transaction_generic_READ),
    static_cast<TransactionCallbackFunctionPointer>(&streamThread_t_base::end_transaction_generic_WRITE),
    static_cast<TransactionCallbackFunctionPointer>(&streamThread_t_base::end_transaction_generic_READ),
    static_cast<TransactionCallbackFunctionPointer>(&streamThread_t_base::end_transaction_generic_WRITE),
    0
  };
...
streamThread_t_base::streamThread_t_base(sc_module_name& module_name, long simulation) :
  papoulis::SystemCBaseModel(module_name),
  m_machineArch(new streamThread_machine_arch()),
  SD_INITIALIZE_PARAMETER(clock, sc_core::sc_time(10, sc_core::SC_NS)),
  SD_INITIALIZE_PARAMETER(generic_clock, clock),
  SD_INITIALIZE_PARAMETER(nominal_voltage, 1),
  SD_INITIALIZE_PARAMETER(mb_debug, false),
...
void streamThread_t_base::update_sync_all() {
  set_sync_all(0);
  set_sync_all(0);
}

void streamThread_t_base::update_pipeline_parameters(esl::sc_sim::pipeline_policy* handle) {
...
void streamThread_t_base::update_sequential_parameters(esl::sc_sim::sequential_policy* handle) {
...
void streamThread_t_base::update_delay_parameters(esl::sc_sim::delay_policy* handle) {
...
void streamThread_t_base::update_bus_parameters(esl::sc_sim::uniform_bus_policy* handle) {
  if (m_bus_vector.size() == 0)
    return;

}

void streamThread_t_base::update_power_parameters(esl::sc_sim::power_policy* handle) {
  if (m_power_vector.size() == 0)
    return;

}

void streamThread_t_base::update_state_power_parameters(esl::sc_sim::state_power_policy* handle) {
...
bool streamThread_t_base::triggerRegistersGotHit(unsigned portIndex, tlm::tlm_generic_payload& trans) {
  mb::utl::Segment<uint64_t> transactionRange(trans.get_address(), trans.get_address() + trans.get_data_length());

  return false;
}

bool streamThread_t_base::portHasRegisters(unsigned portIndex) {
  switch (portIndex) {
...
// Constructor
streamThread_pvt::streamThread_pvt(sc_module_name module_name)
  : esl::sc_sim::PVTBaseModel(module_name, 0),
    streamThread_pvt_param_defaults(this->name()),
    master_1("master_1"),
    slave_a("slave_a"), 
...
{
  m_streamThread_t = streamThread_t_base::create_t("T", 1);
  m_streamThread_pv = new streamThread_pv("PV");
  bind();
}

streamThread_pvt::~streamThread_pvt() {
  delete m_streamThread_t; delete m_streamThread_pv;
}



void streamThread_pvt::bind()
{
...
void streamThread_pvt::b_transport_outside_master_1(tlm::tlm_generic_payload& p, sc_core::sc_time& t)
{
