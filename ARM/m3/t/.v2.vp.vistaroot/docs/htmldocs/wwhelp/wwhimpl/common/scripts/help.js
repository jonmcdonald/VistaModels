...
//RKMGC  this.mbAlwaysSyncTOC +  this.mbAutoSyncTOC   = true instead of false;
  ...
  //MGCSD Added Search param to URL
  this.fGetURLSearchWords              = MGC_GetURLSearchWords;
  this.fClearURLSearchWords            = MGC_ClearURLSearchWords;
  ...
  //MGCSD If we got a search param in the url, need to set tab to Search
  if (URLParams[6] != null && URLParams[6] != "")
  {
...
//MGCRK added function to display title page from Home icon in nav bar
function  MGC_GoToTitlePage()
{
   ...
   DocHandle = DocumentFramePath.DocHandle
   DocumentFramePath.location = "../../../../" + DocHandle + "/" + TitlePage;
}

//
//MGCRK added functions to load document info to support MGC icons in right control pane
...
function GetCurrentBookcaseHandle()
{
   ...
   return DocumentFramePath.BookCaseHandle[0];
}
...
function GetIHUBHandle()
{
   ...
   return DocumentFramePath.IHUBHandle;
}
...
function LinkPDF()
{
   ...
   return DocumentFramePath.PDFLink;
}
function NavEnabled()
{
   var DocumentFramePath = WWHFrame.MGCContent.MGCDocContent;
   ...
   return DocumentFramePath.EnableNavigation;
}
...
function OpenInfoHub()
{
   ...
   {
      window.open("../../../../../infohubs/" + GetCurrentIHUBHandle());
   }
}
...
function DisplayNA()
{
   alert("This feature is not available because a document is not available at this time.");
}

function SubmitFeedback()
{
   var OnSupportNet = "/LocaL";
   var DocLocation = window.location.toString();
   if (DocLocation.indexOf("supportnet\.mentor\.com") != -1)
   {
      OnSupportNet = "/SNeT";
   }
   if (InSearchTips()) { DisplayNA();return; }
   ...
   window.open("http://supportnet.mentor.com/doc_feedback_form?doc_title\=" + DocTitle + " \(Topic is " + Topic + "\, File is " + File + OnSupportNet + "\)&version\=" + SWRelease);
}
function  MGC_Print()
{
...
function DisplayMessage(Msg,width,height) 
{
   ...
   MessageWindow.document.writeln("<html><head></head><body><span style=\"font-family: Verdana, Helvetica, Arial, sans-serif; font-size: " + WWHFrame.BaseBodyFontSize + "px\">\n");
...
//MGCSD Added function to get search terms from URL param
function MGC_GetURLSearchWords()
{
   var SearchWords = "";
   ...
   if (URLParams[6] != null)
   {
      SearchWords = URLParams[6].replace(/^\s*/,"").replace(/\s*$/,"").replace(/ +/g," ").replace(/ +/g," ").replace("\"","").replace("\"","").replace("\%27","").replace("\%27","").replace("\'","").replace("\'","").replace("\%20","");
   }
   //MGCSD need to add parsing of + and %20 into spaces
   return SearchWords;
}

//MGCSD Clears "query=" parameter from the current saved URL
function MGC_ClearURLSearchWords()
{
   var Parts;
   var SearchMarker = "query=";
   var URL = this.mLocationURL;
   ...
   if ( URL.indexOf("?") != -1 )
   {
      ...
      {
         if ( Parts[i].indexOf(SearchMarker) != 0 )
         {
            URL += Parts[i] + "&";
         }
      }
...
{
  //MGCSD Added entry to Array for new Search parameter
  var  URLParams = new Array(null, null, null, null, null, null, null);
  var  Parts;
  ...
  var  TabMarker        = "tab=";
  var  SearchMarker     = "query=";  //MGCSD SearchMarker added for new Search param
  var  MaxIndex;
  var  Index;
  ...
  if (ParamURL.indexOf("?") != -1)
  {
    ...
    for (MaxIndex = Parts.length, Index = 0 ; Index < MaxIndex ; Index++)
    {
      ...
      //MGCSD parse Search parameter
	  if (Parts[Index].indexOf(SearchMarker) == 0)
      {
        URLParams[6] = Parts[Index].substring(SearchMarker.length, Parts[Index].length);
      }
    }
