...
function  WWHSwitch_ParseURLParameters(ParamURL)
{
  ...
  var  Value;
//MGCRK - initialize popup parameter
  SingleParameter = "";
  PopupParameter = "";
  InfoHubTrail     = "";
  ...
  if (ParamURL.indexOf("?") != -1)
  {
    ...
    for (MaxIndex = Parts.length, Index = 0 ; Index < MaxIndex ; Index++)
    {
      if (Parts[Index].indexOf(SingleMarker) == 0)
      {
        ...
        if ((Value == "true") || (Value == "popup"))
        {
          Result[0] = true;
        }
        WWHFrame.popup = "NO";
        PopupParameter = "&popup=false";
        if (Value == "popup")
        {
          Result[0] = true;
          WWHFrame.popup = "YES";
      ...
      else if (Parts[Index].indexOf(IhubMarker) == 0)
      {
        Value = Parts[Index].substring(IhubMarker.length, Parts[Index].length);
        InfoHubTrail="\&ihub=" + Value;
        Result[2] = Value;
      }
      else
      {
...
function  WWHSwitch_Switch(ParamFrameSetURL)
{
  ...
  //
  SwitchURL = ParamFrameSetURL + this.mParameters + PopupParameter + InfoHubTrail;
