...
#if KERNEL_USE_MUTEX
			Mutex_Init(&(pstDriver_->stMutex));
			pstDriver_->bUseMutex = TRUE;
#endif
		}
	}
}
...
*/
UCHAR Driver_Control(DRIVER_STRUCT *pstDriver_,USHORT usID_, void *pvData_)
{
	...
	if (pstDriver_ != NULL)
	{
		...
		if ((pstDriver_->pfControl != NULL) && (pstDriver_->eState > DRIVER_CLOSE))
		{
			...
			{
				BOOL bCacheMutex;
				bCacheMutex = pstDriver_->bUseMutex;
				if (bCacheMutex)
				{
					Mutex_Claim(&(pstDriver_->stMutex), TIME_FOREVER);
				}
#endif
				...
				if (bCacheMutex)
				{
					...
					Mutex_Release(&(pstDriver_->stMutex));
				}
			}
#endif
