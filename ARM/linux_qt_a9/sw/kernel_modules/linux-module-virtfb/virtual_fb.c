...
/* Vista LinuxFrameBufferDisplay Register definitions */

#define LFBD_BASE 0x10025000UL
#define LFBD_SMEM_START     0x0
#define LFBD_SMEM_LEN       0x4
#define LFBD_XRES           0x8
#define LFBD_YRES           0xC
#define LFBD_BITS_PER_PIXEL 0x10

static void* lfbd_virt = 0;
...
static int virtfb_set_par(struct fb_info *fbi)
{
	...
	if (!fbi->fix.smem_start || (mem_len > fbi->fix.smem_len)) {
		...
		if (virtfb_map_video_memory(fbi) < 0)
        	...
        	iowrite32(mem_len, lfbd_virt + LFBD_SMEM_LEN);
        }
...
static int virtfb_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	if ((var->bits_per_pixel != 32) && (var->bits_per_pixel != 24) &&
        ...
        iowrite32(var->xres, lfbd_virt + LFBD_XRES);
        iowrite32(var->yres, lfbd_virt + LFBD_YRES);
        iowrite32(var->bits_per_pixel, lfbd_virt + LFBD_BITS_PER_PIXEL);
...
static int virtfb_map_video_memory(struct fb_info *fbi)
{
	...
	fbi->screen_size = fbi->fix.smem_len;
        ...
        iowrite32(fbi->fix.smem_start, lfbd_virt + LFBD_SMEM_START);
        iowrite32(fbi->fix.smem_len, lfbd_virt + LFBD_SMEM_LEN);
...
static int virtfb_register(struct fb_info *fbi, unsigned int id)
{
	...
	fbi->var.xres_virtual = fbi->var.xres = 640;
	fbi->var.yres_virtual = fbi->var.yres  = 480;
...
int __init virtfb_init(void)
{
        ...
        /*
         * Obtain access to the Vista model registers
         */
        lfbd_virt = ioremap(LFBD_BASE, 0x1000);
