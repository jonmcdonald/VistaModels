...
#include "PropCan_model.h"
#include <queue>
#include <vector>

using namespace tlm;

class PropCan_pv : public PropCan_pv_base {
 public:
  typedef esl::tlm_types::Address mb_address_type;

  SC_HAS_PROCESS(PropCan_pv);
  PropCan_pv(sc_module_name module_name);
    ...
    unsigned char * m_data;
        ...
        : m_ident(address), m_size(size)
      {
        ...
        if (m_tokenptr && m_tokenptr->hasField("CANDataPtr")) {
          ...
          cout << "Error: PropCan_pv DataType constructor.\n";
        }
      }
